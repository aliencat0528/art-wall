...
- **走進展場** — 第三種呈現方式：第一人稱走廊，作品掛在左右牆上沿透視後退到消失點，共用一條掛畫線、寬度隨比例（橫幅就是橫的，不裁切），地板有一道道往深處推的光波（走動時加速）。方向鍵或 `BACK`／`WALK ON` 一次走一件，走到面前自動換上大圖。刻意不是預設入口——訪客要一眼看完整面牆，走廊是給願意逛的人。窄螢幕照樣進得去，只是走廊收窄成一次看一件（已知的體驗降級）；系統要求減少動態時不提供
