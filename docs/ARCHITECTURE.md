...
| `components/GalleryHall.vue` | 走廊模式的房間（四個面）、相機推進、光波地板、近景換大圖、方向鍵與橫向滑動 | 幾何算式（在 `utils/hall.ts`）、誰能進走廊（由 `App.vue` 判定） |
...
```mermaid
flowchart LR
  K["方向鍵 / BACK / WALK ON<br/>橫向滑動（手機主要操作）"] --> W["walk(delta)"]
...
**手機的主要操作是橫向滑動，不是 HUD 那三顆鈕**（MR-020）。手機沒有方向鍵，
而三顆鈕在窄螢幕先前只有 28～82 寬、29～30 高，全都不到 44px 觸控下限。
滑動用 `touchstart`／`touchend`，**絕不碰 `setPointerCapture`**——指標捕獲會把後續
`click` 改派給容器，作品就全部點不開（MR-014 踩過）。門檻 60px 且要求橫移大於縱移，
否則點擊時的手抖會變成「想點作品卻走了一步」，直向捲頁也會被誤判成走路。
...
| 走廊（選配，全尺寸） | `.app__main` 的剩餘空間（`flex: 1`） | 不吃 `--rail-h`——那是為「一排卡片＋上下留白」設計的帶狀區，走廊用它會在上下各留一段空白，地板接不到頁尾。房間的四個面與作品全部在 3D 場景內，`transform` 不改 layout box，故不參與版面計算。窄螢幕只收窄走廊半寬與作品寬，**幾何與相機不變** |

三種模式都不讓圖片的固有尺寸參與版面計算，這是「圖片不會撐破版面」的根本原因。

**視窗高度一律用 `dvh`，不用 `vh`**（`#app`、`.app`、`--rail-h`、`.intro`；MR-020）。

iOS Safari 的 `100vh` ＝ **large viewport**（工具列收起後的高度），不是當下看得到的高度。
差額實測 126px（iPhone 12：版面 790／可見 664）。走廊模式沒有可捲的內容、整頁就是一屏，
於是**站在版面底部的東西會整個落在可見區外**——走廊 HUD 落在 668～721、開場的跳過鈕
底邊在可見區下方 110px，按下去按到的是 Safari 的工具列。症狀讀起來就是「按鈕失效」。

⚠️ **這個 bug 在桌機與 Chromium 的行動模擬都重現不了**（兩者只有一個視窗高度，
`dvh` 恆等於 `vh`）。要驗證就**把版面容器設成 790、拿 664 當可見底線去量元素座標**。

`--rail-h` 是自訂屬性，**不寫 `62vh` 的退路**：自訂屬性照單全收任何 token，
舊瀏覽器會存下 `62dvh` 再在代入時整條失效，比沒有退路更糟。本專案早就大量用
`color-mix`（Safari 16.2），支援度比 `dvh`（Safari 15.4）還晚，過得了前者就過得了後者。
...
**走廊裡的作品是正對螢幕的看板，不貼在牆上**（MR-018）。它們仍在 3D 場景裡、
仍有深度與透視縮放，但面永遠平行於螢幕，所以任何角度都是完整正面。
空間感由走廊本身提供，作品只負責被看清楚。

尺寸給的是**上限框**（`--piece-max-w` / `--piece-max-h`），比例由圖自己決定：
橫幅被寬度限制、直幅被高度限制，兩種都不裁切。**`--piece-max-h` 是直幅存在感的唯一旋鈕**
——橫幅早被寬度封住，調高度只影響直幅。

作品**沒有裱框**：padding 為 0，邊界只由光界定。
中間試過「透明底 + padding」，結果**還是黑框**——作品是亮白的，padding 區透出的牆
再暗一點都會被讀成一圈黑邊，這是對比造成的，跟背景是什麼顏色無關。

界定邊界的光是**疊印雙光框**（`.piece__reg`，MR-019）：`--accent` 與 `--counter`
兩道邊框往反方向錯位、`mix-blend-mode: screen` 讓相交處加成出白光，
與牆面卡片（`WorkCard` 的 `.card__reg`）是同一套語彙。走廊比牆面粗一階（2px）——
作品被透視縮放過，1px 在遠端細到看不出顏色，霓虹感就只剩一圈白光暈。

**錯位只在 hover／focus 時收攏，不對「當前那一件」收攏**：兩道有色邊一對齊就重疊成
一條白線，走到作品面前反而最沒有霓虹感。錯位是效果本體，不是「還沒對準」的過渡。
...
看板是平行螢幕的平面、牆是縱深平面，兩者一定會相交——作品橫向只要超出牆的位置，
靠近相機的那半就跑到牆外，**被牆斜切掉一角**。改任何一個值都要重算這條。
（作品已無裱框 padding，舊版式子裡的 36px 隨之消失。）

**還有第二條**：`--piece-max-h + 展籤約 20px < 走廊淨高`（`--ground` − `--ceil`）。
超過就會穿出地板，被地板平面斜切掉一條。

⚠️ **第三條是螢幕邊緣，不是牆**（MR-019）：

```
螢幕半寬 − --lateral × 透視縮放 − 作品螢幕半寬  >  0
```

前兩條管的是「作品有沒有撞到走廊」，這條管「作品有沒有撞到畫面」，兩者互不涵蓋。
**只有走到那一件面前才會壞**：縮放（`620 / (620 + HALL_LEAD)` ≈ 1.32）只作用在當前
那件身上，站遠看一切正常。`--lateral: 380` 時 1280×720 下橫幅走到面前右緣是 −109px，
直幅離左緣只剩 23px；收到 250 之後兩種版位在 1280／1440／1920 三種寬度都有留白。

**決定值的是最窄的桌機寬度**：作品尺寸吃 `36vh`，寬度縮、高度不縮，
所以 1280 必然比 1920 先破。留白也要算進外圍那圈光（mat 78px ＋ 疊印框 34px），
只讓圖不被切、光貼在邊上，讀起來仍是「頂到邊」。

由 `e2e/hall.spec.js`「走到直幅與橫幅面前，作品都不會被畫面左右緣切掉」守住
（桌機 1280×720 與手機 360×780 各跑一次）。既有的「不撐出捲軸」測試**抓不到這一條**
——`.hall__viewport` 是 `overflow: hidden`，被切掉的部分不會變成捲軸。

**手機由這條決定版位，不是由撞牆那條**：360 寬時螢幕半寬只有 180，
舊的 `--piece-max-w: 260` 換算到螢幕是 343px 比整個螢幕還寬，`--lateral` 收到 0 也塞不下。
收成 20／160／190 之後是「幾乎置中、一次看一件」，即 MR-017 描述的降級樣貌。
作品中心點落在視窗外不只是難看——**點不到**，違反 MR-017 三條硬要求之一。

⚠️ **命中測試不會自己跟著 3D 深度走**：`.piece__reg` 的 `mix-blend-mode` 讓 3D 子樹在
hit-test 時被壓平，改以 DOM 順序決定誰先被打到，於是遠處那件（落在消失點附近＝當前那件的
正中央）會攔掉近處的點擊，而**畫面上完全看不出來**。兩道防線：疊印框一律
`pointer-events: none`（純裝飾），`.piece` 的 `z-index` 由深度倒著給（近的高）。

**房間的四個面是半透明疊加，不是實心板**：`to right`（對牆而言就是由近到遠）的 alpha
從 0.84 收到 0.16，消失點附近讓背景光氛透出來。深度靠亮度差讀出來，不是再畫一層更黑的。
代價是 MR-017「走廊不透光」順帶解掉的第八條限制部分失效——顆粒與色場會透進來一點。
